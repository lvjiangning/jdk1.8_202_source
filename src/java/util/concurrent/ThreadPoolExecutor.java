...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    // 最大支持线程数 2^29-1  00011111111111111111111111111111
...
// 在threadPoolExecutor初始化的时候赋值,acc对象是指当前调用上下文的快照，其中包括当前线程继承的AccessControlContext和任何有限的特权范围，使得可以在稍后的某个时间点(可能在另一个线程中)检查此上下文。
     ...
     */
    /**
     * 继承AQS的原因： 线程只有两个状态，一个是独占表示线程正在运行，一个是不加锁空闲状态，用AQS的state区分，
     * 这里为啥不用ReentrantLock，主要原因是ReentrantLock是允许重入的。
     */
            ...
            implements Runnable
    {
        ...
        Worker(Runnable firstTask) {
            ...
            this.thread = getThreadFactory().newThread(this); //通过线程工厂创建工作线程 这个对象的thread 指向被封装后的work
        }
        ...
        for (;;) {
            ...
            if (workerCountOf(c) != 0) { // Eligible to terminate
                /*
                  ...
                  这个时候线程池一定是STOP的状态或者SHUTDOW并队列不为空,这两种情况要么就是尝试终止因此这个时候尝试中断一个空闲worker
                 */
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (;;) {
            ...
            int rs = runStateOf(c); //得到当前运行状态

            // Check if queue empty only if necessary.
            if (rs >= SHUTDOWN  // 如果线程池状态是SHUTDOWN、STOP、TIDYING、TERMINATED就不允许提交。
            ...
            for (;;) {
                ...
                // 是否超过当前约定的最大值，超过就拒绝加入，直接返回，这里你要看core传的是什么，也就是当前处于哪一种情形，是核心线程池没用完还是咋的。
                if (wc >= CAPACITY ||
        ...
        Worker w = null;
        try {
            //构建worker，并将当前任务赋值给当前worker,这个地方要看一下new Worker的源码，如下，你会发现会直接new一个线程给当前worker对象
            ...
            if (t != null) {
                ...
                try {
                    ...
                    if (rs < SHUTDOWN //如果当前状态是<SHUTDOWN也就是RUNNING状态
                            ||
                            //或者状态是SHUTDOWN并且当前任务是空的，这种就是单纯想新搞一个worker（thread），比如前面说的场景，阻塞队里里面还有，但当前已经是不允许提交的状态了。
                            (rs == SHUTDOWN && firstTask == null)) {
                        if (t.isAlive()) // 预先检查 t 是否可启动
                            throw new IllegalThreadStateException(); //刚刚new的线程怎么就启动了，这种就抛异常，错误的线程状态异常。
                        workers.add(w); //增加wokrer，这也就是为啥上面加锁的缘故，至于为啥要用hashSet，可以看上文成员变量wokers的说明。
                        int s = workers.size(); //得到当前worker的总数
                        if (s > largestPoolSize) //当前worker的总数如果大于之前记录的线程池大小,则刷新值
                            largestPoolSize = s;
    ...
    private Runnable getTask() {
        ...
        for (;;) { // 死循环
            ...
            if ((wc > maximumPoolSize || (timed && timedOut))
                ...
                //这里不一样,线程池的状态可能是RUNNING状态,多减一次,可能导致任务获取不到worker去跑.
            ...
            try {
                ...
                //因为阻塞队列poll take都是使用快速响应中断的加锁方式(lockInterruptibly()),因此需要捕获中断异常.并且这方法是用的Thread.interrupted()判断的,有个特点会擦除中断状态,这就说明getTask方法是不响应中断的.
                timedOut = false;
            }
        }
    }
     ...
     */
    /**
     *  runWorker方法属于只要开启了一个线程,就一直循环执行getTask, getTask方法是可能阻塞的,当task==null&&getTask==null 这种就才会关闭当前任务.
     * @param w 当前工作线程
     */
    final void runWorker(Worker w) {
        ...
        try {
            //任务不等于空 或者 getTask() 得到任务不为空，如果getTask()取不到任务，会阻塞在这里
            while (task != null || (task = getTask()) != null) {
                ...
                if ((runStateAtLeast(ctl.get(), STOP)   //线程池的状态>=STOP
                        ||
                        (Thread.interrupted() && runStateAtLeast(ctl.get(), STOP))) //线程已中断并且线程状态>=STOP
                        && !wt.isInterrupted()) { // 并且当前线程没有被中断。
                    wt.interrupt(); //中断当前线程
                }
                try {
            ...
            // 一定要注意。执行到这里说明task == null并且getTask()返回null。
            // 说明当前线程池中不需要那么多线程来执行任务了，可以把多于corePoolSize数量的工作线程干掉,也可能被中断被SHUTDOWN等,也会导致getTask返回Null
            // completedAbruptly = true ,在执行task.run()过程中出现了异常，导致 completedAbruptly 没有置false位
    ...
    public void execute(Runnable command) {
        ...
        if (workerCountOf(c) < corePoolSize) {
            //创建新的工作线程 并且 将当前command作为其第一个任务
    ...
    public boolean isTerminating() {
        int c = ctl.get();
        //看看状态是不是TIDYING/STOP/SHUTDOWN如果是这三种状态就返回true,此时线程池正在停止,不是这三种状态并且不是RUNNING状态,那么就是TERMINATED状态,返回false,此时线程池已经终止了.
     ...
     * @return the number of threads
     */
    //返回正在执行任务的大致线程数 , 注意: 不是精确值, 所以目前看作用就只有展示用, 不能作为临界条件使用.
     ...
     * @return the number of tasks
     */
    // 返回已执行完成的任务的大致总数,为啥不精确,原因与getActiveCount相似, 就不分析了.
